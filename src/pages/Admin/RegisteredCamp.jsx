...
const RegisteredCamp = () => {
   ...
   const fetchCamps = async ({ queryKey }) => {
    ...
    const response = await axiosSecure.get(`/participant?page=${currentPage}&size=${itemsPerPage}&search=${search}`);
...
//   console.log("Camps",camps);
  ...
  const fetchCampCount = async ({ queryKey }) => {
    ...
    const response = await axiosSecure.get(`/participant_count?search=${search}`);
    return response.data;
  };
...
const handleConfirmation = async (campId , status ) => {

    if( status == "Pending"){

    Swal.fire({
        title: 'Are you sure want to confirm it?',
        text: "You won't be able to undo this! It will be confirmed",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            ...
            try {
                 await axiosSecure.patch(`/confirm-participant/${campId}`)
                Swal.fire('Confirmed  Successful')
                refetch();
             ...
             <tbody>
                 ...
                 <tr key={camp?._id} className=" bg-blue-100 text-black">
                     <td className="w-3/10">
                         <div className="">
                             <div>
                                 <div className="font-bold">{camp?.ParticipantName}</div>
                     ...
                     <button 
                          ...
                          onClick={()=> handleConfirmation(camp?._id , camp?.ConfirmationStatus)} className={`text-black p-2 font-bold rounded-lg hover:scale-125 ${camp?.ConfirmationStatus === 'Confirmed' ? 'bg-green-500' : ''}`}>
                             {camp?.ConfirmationStatus} </button>
                     ...
                     <td className="w-1/10">
                         ...
                         disabled={camp?.ConfirmationStatus === 'Confirmed' && camp?.PaymentStatus === 'Paid'}
                          onClick={()=> handleDelete(camp?._id)} className="btn bg-red-500
                             text-white">
